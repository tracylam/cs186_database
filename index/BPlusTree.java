...
public class BPlusTree {
    ...
    public void insertKey(DataBox key, RecordID rid) {
        ...
        BPlusNode root = BPlusNode.getBPlusNode(this, rootPageNum);
        LeafEntry newEnt = new LeafEntry(key, rid);
        InnerEntry result = root.insertBEntry(newEnt);
        ...
        if (result != null) {
            ...
            List<BEntry> newRootEntries = newRoot.getAllValidEntries();
            newRootEntries.add(result);
...
//>>>>>>> c8dc87c88cba4ba3c1533120d3f7a7c36398be2d
    ...
    private class BPlusIterator implements Iterator<RecordID> {
        ...
        Stack<BPlusNode> nodes = new Stack<BPlusNode>();


        public BPlusIterator(BPlusNode root) {
            if (root.isLeaf()) {
                Iterator<RecordID> leafRec = ((LeafNode) root).scan();
                while (leafRec.hasNext()) {
                    records.add(leafRec.next());
                }
            } else {
                nodes.push(root); //add first item to stack
                while (!nodes.isEmpty()) {
                    while (!root.isLeaf()) {
                        List<BEntry> rootEnt = root.getAllValidEntries();
                        Collections.reverse(rootEnt);
                        int first = ((InnerNode) root).getFirstChild();

                        for (BEntry ent : rootEnt) {
                            int pgNum = ent.getPageNum();
                            BPlusNode toAdd = (BPlusNode.getBPlusNode(root.getTree(), pgNum)); //adds child pages to stack
                            nodes.push(toAdd); //add to stack
                        }
                        nodes.add(BPlusNode.getBPlusNode(root.getTree(), first));
                        root = nodes.pop(); //root is now it's chld
                    }
                    Iterator<RecordID> leafRec = ((LeafNode) root).scan();
                    while (leafRec.hasNext()) {
                        records.add(leafRec.next());
        ...
        public BPlusIterator(BPlusNode root, DataBox key, boolean scan) {
            if (scan) {
                if (root.isLeaf()) {
                    Iterator<RecordID> leafRec = ((LeafNode) root).scanFrom(key);
                    while (leafRec.hasNext()) {
                        records.add(leafRec.next());
                    }
                } else {
                ...
                if (root.isLeaf()) {
                    if (((LeafNode) root).containsKey(key)) {
                        Iterator<RecordID> leafRec = ((LeafNode) root).scanForKey(key);
                        while (leafRec.hasNext()) {
                            records.add(leafRec.next());
                        }
                    }
